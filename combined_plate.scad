...
// spool for display with inner spool defined.
...
translate([-155,112,25]) linear_extrude(height = 20) text(text="Elegoo", size = 30);
//
//difference() {
//
...
group() {
	...
	translate([-spool_width*(len(elegoo_pla))/2,-spool_diameter*3,0])
	...
	for(i = [0:len(elegoo_petg_rapid)-1]) {
		...
		color("Green") translate([spool_width*i,-30,30]) rotate([0,90,0]) spool();
	}
